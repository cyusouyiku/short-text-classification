...
class TextClassificationModel(nn.Module):
    def __init__(self, config):
        ...
        self.lambda_value = config.get('lambda_value', 1.0)  # weight for value loss
        
        # Unified embedding: RoBERTa + Word2Vec static + Positional (Section 2.2)
        ...
        # CapsuleNet - receives embeddings from unified layer
        ...
        # MultiscaleTransformer - receives embeddings from unified layer
        ...
        # Paper 2.5: multi-head self-attention over fused representations
    ...
    def get_tokenizer(self):
        """Return RoBERTa tokenizer for data preprocessing"""
        return self.embedding_layer.tokenizer
        
    def _extract_features(self, input_ids, attention_mask=None):
        """Extract features: unified embedding -> CapsuleNet + MultiscaleTransformer"""
        # Get combined embeddings (RoBERTa + Word2Vec + Positional)
        embeddings = self.embedding_layer(input_ids, attention_mask)  # (B, L, embed_dim)
        
        batch_size, seq_len, _ = embeddings.size()
        
        # Transformer branch
        transformer_features = self.transformer(embeddings, attention_mask)  # (B, L, hidden_dim)
        
        # Capsule branch
        capsule_features = self.capsule(embeddings, attention_mask)  # (B, num_classes, num_primary_channels)
    ...
    def forward(self, input_ids, attention_mask=None, positions=None, labels=None):
        """
        Forward pass. 
        Per Algorithm 1: policy gradient + value loss only (no cross-entropy).
        """
        features, seq_len = self._extract_features(input_ids, attention_mask)
        
        attended_features, attention_weights = self.self_attention(features, attention_mask)  # Multi-head fusion (paper 2.5)
        ...
        if labels is not None:
            # Actor-Critic: sample action, compute advantage, policy gradient + value loss
            # Sample action a from π(a|s) during training
            dist = torch.distributions.Categorical(probs=policy_probs)
            sampled_action = dist.sample()
            
            # Binary reward: r = 1 if correct, 0 otherwise
            ...
            # Ltotal = Lpolicy + λ * Lvalue (no cross-entropy per paper)
    ...
    def predict(self, input_ids, attention_mask=None, positions=None):
        """Predict: use argmax at test time (Algorithm 1)"""
        self.eval()
        with torch.no_grad():
