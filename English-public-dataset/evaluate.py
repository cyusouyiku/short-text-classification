...
def compute_metrics(predictions, labels, class_names=None):
    ...
    # 手动计算macro和weighted指标
    macro_precision = np.mean(precision_per_class)
    macro_recall = np.mean(recall_per_class)
    macro_f1 = np.mean(f1_per_class)
    
    total_support = max(total_support, 1)  # 避免除零
    ...
    metrics = {
        ...
        'precision': {
            'micro': float(accuracy),  # 在多分类中，micro precision等于accuracy
        ...
        'recall': {
            'micro': float(accuracy),  # 在多分类中，micro recall等于accuracy
        ...
        'f1': {
            'micro': float(accuracy),  # 在多分类中，micro F1等于accuracy
