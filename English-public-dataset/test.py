...
def parse_args():
    ...
    parser.add_argument('--num_classes', type=int, default=4, help='Number of classes (9 for this dataset)')
...
def visualize_model_architecture(model, input_size, output_dir):
    ...
    Args:
        ...
        input_size: Tuple of (batch_size, seq_length)
        output_dir: Directory to save the visualization
    """
    try:
        # Create dummy input
        batch_size, seq_length = input_size
        input_ids = torch.randint(0, model.vocab_size, (batch_size, seq_length)).to(next(model.parameters()).device)
        attention_mask = torch.ones((batch_size, seq_length)).to(next(model.parameters()).device)
        ...
        # Save visualization
        viz_path = os.path.join(output_dir, 'model_architecture')
        dot.render(viz_path, format='png', cleanup=True)
        print(f"Model architecture visualization saved to {viz_path}.png")
        ...
        with open(summary_path, 'w') as f:
            ...
            f.write(f'- Number of Transformer Heads: {model.transformer.num_heads}\n')
            f.write(f'- Number of Transformer Layers: {len(model.transformer.transformer_layers)}\n\n')
...
def main():
    try:
        ...
        try:
            ...
            logger.info("Continuing with model evaluation...")
