...
class CapsuleNet(nn.Module):
    """
    Capsule network - accepts pre-computed embeddings (RoBERTa+Word2Vec+Position).
    """
    def __init__(self, embedding_dim, num_primary_units=8, 
                 num_primary_channels=64, num_classes=4, num_routing=3, 
                 dropout=0.5):
    ...
    def forward(self, x, attention_mask=None):
        ...
        for r in range(self.num_routing):
            ...
            if r < self.num_routing - 1:
                ...
                # Adjust dimensions for agreement calculation
                u_hat_reshaped = u_hat.view(batch_size, self.num_primary_units, self.num_classes, -1)  # (batch_size, num_primary_units, num_classes, seq_len * num_primary_channels)
                v_expanded_reshaped = v_expanded.view(batch_size, 1, self.num_classes, -1)  # (batch_size, 1, num_classes, seq_len * num_primary_channels)
                
                # Calculate agreement
                agreement = torch.sum(u_hat_reshaped * v_expanded_reshaped, dim=-1)  # (batch_size, num_primary_units, num_classes)
                
                # Expand agreement to match b's dimensions
                agreement = agreement.unsqueeze(-1).expand(-1, -1, -1, seq_len)  # (batch_size, num_primary_units, num_classes, seq_len)
