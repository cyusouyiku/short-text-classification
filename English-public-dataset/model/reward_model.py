...
class TextClassificationModel(nn.Module):
    ...
    def visualize(self, save_path='model_architecture', batch_size=1, seq_length=128):
        try:
            ...
            tokenizer = self.embedding_layer.tokenizer
            dummy_texts = ["This is a sample text for visualization."] * batch_size
            encodings = tokenizer(dummy_texts, padding=True, truncation=True, max_length=seq_length, return_tensors="pt")
            input_ids = encodings['input_ids']
            attention_mask = encodings['attention_mask'].float()
            outputs = self.forward(input_ids, attention_mask)
            from torchviz import make_dot
            dot = make_dot(outputs['logits'], params=dict(self.named_parameters()), show_attrs=True, show_saved=True)
            dot.attr(rankdir='TB')
