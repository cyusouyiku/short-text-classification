...
class MultiscaleTransformer(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_heads=6, 
        ...
        self.num_heads = min(num_heads, embedding_dim // 64)  # 确保每个head至少64维
        while embedding_dim % self.num_heads != 0:
            self.num_heads -= 1
        assert self.num_heads > 0, "embedding_dim太小，无法支持多头注意力"
        ...
        self.transformer_layers = nn.ModuleList([
            MultiscaleTransformerLayer(
                ...
                num_heads=self.num_heads * (2 ** min(i // 2, 2)),  # 逐层增加注意力头数
